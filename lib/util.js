...
exports.debugConfigParamsOld = (config) => {
  debug(`util.debugConfigParams()`);

const spacer = '\n                                  : ';
/*
  if (config.params.config) {
    debug(`config.params.config.path         : ${config.params.config.path}`);
    debug(`config.params.config.bucket       : ${config.params.config.bucket}`);
  }
  if (config.params.templates) {
    debug(`config.params.templates.path      : ${config.params.templates.path}`);
    debug(`config.params.templates.bucket    : ${config.params.templates.bucket}`);
  }
  if (config.params.functions) {
    debug(`config.params.functions.path      : ${config.params.functions.path}`);
    debug(`config.params.functions.bucket    : ${config.params.functions.bucket}`);
  }
  if (config.params.scripts) {
    debug(`config.params.scripts.path        : ${config.params.scripts.path}`);
    debug(`config.params.scripts.bucket      : ${config.params.scripts.bucket}`);
  }

  if (config.params.system) {
    debug(`config.params.system.Name         : ${config.params.system.Name}`);
    debug(`config.params.system.Code         : ${config.params.system.Code}`);
    debug(`config.params.system.Default      : ${config.params.system.Default}`);
    debug(`config.params.system.Source       : ${config.params.system.Source}`);
  }
  if (config.params.account) {
    debug(`config.params.account.Name        : ${config.params.account.Name}`);
    debug(`config.params.account.LongName    : ${config.params.account.LongName}`);
    debug(`config.params.account.Alias       : ${config.params.account.Alias}`);
    debug(`config.params.account.Id          : ${config.params.account.Id}`);
    debug(`config.params.account.Default     : ${config.params.account.Default}`);
    debug(`config.params.account.Source      : ${config.params.account.Source}`);
  }
  if (config.params.environment) {
    debug(`config.params.environment.Name    : ${config.params.environment.Name}`);
    debug(`config.params.environment.Code    : ${config.params.environment.Code}`);
    debug(`config.params.environment.Active  : ${config.params.environment.Active}`);
    debug(`config.params.environment.Default : ${config.params.environment.Default}`);
    debug(`config.params.environment.Source  : ${config.params.environment.Source}`);
  }
  if (config.params.region) {
    debug(`config.params.region.Name         : ${config.params.region.Name}`);
    debug(`config.params.region.LongName     : ${config.params.region.LongName}`);
    debug(`config.params.region.Code         : ${config.params.region.Code}`);
    debug(`config.params.region.Active       : ${config.params.region.Active}`);
    debug(`config.params.region.Default      : ${config.params.region.Default}`);
    debug(`config.params.region.Source       : ${config.params.region.Source}`);
  }
  if (config.params.user) {
    debug(`config.params.user.Name           : ${config.params.user.Name}`);
    debug(`config.params.user.FullName       : ${config.params.user.FullName}`);
    debug(`config.params.user.Email          : ${config.params.user.Email}`);
    debug(`config.params.user.Phone          : ${config.params.user.Phone}`);
    debug(`config.params.user.Default        : ${config.params.user.Default}`);
    debug(`config.params.user.Source         : ${config.params.user.Source}`);
  }
  if (config.params.profile) {
    debug(`config.params.profile             : ${config.params.profile}`);
  }
*/

  debug(`config.params.verbose             : ${config.params.verbose}`);
  debug(`config.params.confirm             : ${config.params.confirm}`);

  if (config.params.stack.template) { // Only set for create stack command
    debug(`config.params.prerequisite        : ${config.params.prerequisite}`);
    debug(`config.params.lambda              : ${config.params.lambda}`);
    debug(`config.params.policy              : ${config.params.policy}`);
    debug(`config.params.monitor             : ${config.params.monitor}`);
    debug(`config.params.wait                : ${config.params.wait}`);
  }

  if (config.params.stack) {
    debug(`config.params.stack.name          : ${config.params.stack.name}`);
    if (config.params.stack.template) {
      debug(`config.params.stack.template.name : ${config.params.stack.template.name}`);
      debug(`config.params.stack.template.path : ${config.params.stack.template.path}`);
      debug(`config.params.stack.template.url  : ${config.params.stack.template.url}`);
    }
    if (config.params.stack.policy) {
      debug(`config.params.stack.policy.path   : ${config.params.stack.policy.path}`);
      debug(`config.params.stack.policy.url    : ${config.params.stack.policy.url}`);
    }
    if (config.params.stack.parameters) {
      debug(`config.params.stack.parameters    : ${JSON.stringify(config.params.stack.parameters, null, 2).replace(/\n/g, spacer)}`);
    }
    if (config.params.stack.tags) {
      debug(`config.params.stack.tags          : ${JSON.stringify(config.params.stack.tags, null, 2).replace(/\n/g, spacer)}`);
    }
    if (config.params.stack.capabilities) {
      debug(`config.params.stack.capabilities  : ${JSON.stringify(config.params.stack.capabilities, null, 2).replace(/\n/g, spacer)}`);
    }
  }
  if (config.params.waitInterval)
    debug(`config.params.waitInterval        : ${config.params.waitInterval}`);

  if (config.params.phases) {
    debug(`config.params.phases              : ${config.params.phases.join(',')}`);
  }
  if (config.params.steps) {
    debug(`config.params.steps               : ${config.params.steps.join(',')}`);
  }
}
...
exports.extractTemplateParameters = (templateBody) => {
...
exports.extractTemplateCapabilities = (templateBody) => {
  debug(`util.extractTemplateCapabilities()`);

  if (/AWS::IAM::(ManagedPolicy|Group|Role|User)/.test(templateBody)) {
    debug('- Found IAM resources');
    if (/\n *(ManagedPolicy|Group|Role|User)Name:/.test(templateBody)) {
...
exports.createLambdaDeploymentPackage = (config, name) => {
  ...
  const functionFolderPath = path.join(config.params.functions.path, name);
  const zipFilePath = path.join(config.params.functions.path, name + '.zip');
