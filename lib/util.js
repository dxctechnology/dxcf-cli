...
exports.formatCreateStackCommand = (config, createStackParams) => {
  debug(`util.formatCreateStackCommand()`);
/*
  let config = {};
  config.template = {};
  config.template.Name = 'Common-Topics';

  let createStackParams = {
    StackName: 'Topics',
    TemplateURL: 'https://s3-us-west-2.amazonaws.com/templates-mjct-us-west-2/Common-Topics.yaml',
    Parameters: [],
    Tags: [
      { Key: 'Company', Value: 'MJCConsulting' },
      { Key: 'System', Value: 'Prototype' },
      { Key: 'Environment', Value: 'Testing' },
      { Key: 'Owner', Value: 'mcrawford@mjcconsulting.com' }
    ],
    Capabilities: [],
    DisableRollback: true
  }

source command phrase        environment/default
       dxcf stack create
G --verbose [false]            env.VERBOSE
G --confirm [false]            env.CONFIRM
C --prerequisite [false]       env.PREREQUISITE
C --lambda [false]             env.LAMBDA
C --policy [false]             env.POLICY
C --monitor [false]            env.MONITOR
C --wait [false]               env.WAIT

G --templates <path>           env.TEMPLATES
G --functions <path>           env.FUNCTIONS
G --scripts <path>             env.SCRIPTS
G --config <path>              env.CONFIG

G --system <SYSTEM>            env.SYSTEM
G --account <ACCOUNT>          env.ACCOUNT
G --environment <ENVIRONMENT>  env.ENVIRONMENT
G --region <REGION>            env.REGION
G --user <USER>                env.USER

C --stack-name <stack>
C --template-name <template>
C --wait-interval <seconds>    15

Maybe delete?
G --owner <OWNER>              env.OWNER
G --company <COMPANY>          env.COMPANY
G --location <LOCATION>        env.LOCATION



/*
  let   formattedCommand = 'dxcf stack create --stack-name ' + createStackParams.StackName + ' \\\n' +
                           '                  --template-name ' + config.template.Name + ' \\\n';

                           '                  --parameters ParameterKey=TopicPrefix,ParameterValue=MJCT \\\n' +
                           '                  --tags Key=Owner,Value="mcrawford@mjcconsulting.com" \\\n' +
                           '                         Key=Company,Value="$company" \\\n' +
                           '                         Key=System,Value="$system" \\\n' +
                           '                         Key=Environment,Value=$environment \\\n' +
                           (createStackParams.Capabilities && createStackParams.Capabilities.length > 0) ?
