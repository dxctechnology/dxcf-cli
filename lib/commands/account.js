...
const timer = (ms) => {
  debug(`account.timer(${ms})`);
  return new Promise(res => setTimeout(res, ms));
}

const calculateETag = (body) => {
  debug(`account.calculateETag()`);
...
const formatAccountCommand = (subcommand, config, params) => {
  debug(`account.formatAccountCommand()`);
