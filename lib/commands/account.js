...
const formatCreateStackCommand = (config, createStackParams) => {
  debug(`stack.formatCreateStackCommand()`);

  let formattedCommand = 'dxcf stack create ';

  const formattedSpacer = ' '.repeat(18);
  ...
  const formattedParameters = util.formatParameters(createStackParams.Parameters);
  const formattedTags = util.formatTags(createStackParams.Tags);
...
exports.describeAccounts = async (config) => {
  debug(`account.describeAccounts()`);

  console.log(`account.describeAccounts() - WIP: Not yet implemented, code copied from stack.js to setup structure, but not yet modified.`);
  return;
...
exports.createAccount = async (config) => {
  debug(`account.createAccount()`);

  console.log(`account.createAccount() - WIP: Not yet implemented, code copied from stack.js to setup structure, but not yet modified.`);
  return;
  ...
  try {
    ...
    if (!stackExists) {
      ...
      if (config.verbose) {
        console.log(chalk.bold(formatCreateStackCommand(config, createStackParams)));
        debug(`Parameters: ${util.formatMergedParameters(mergedParameters)}`);
        debug(`Tags: ${util.formatMergedTags(mergedTags)}`);
