...
exports.describeStacks = async (config) => {
  ...
  try {
    ...
    if (config.verbose) {
      console.log(`System: ${config.system.Name} (${config.system.Code})`);
      console.log(`Account: ${config.account.Name} (${config.account.Id}:${config.account.Alias})`);
      console.log(`Environment: ${config.environment.Name} (${config.environment.Code})`);
