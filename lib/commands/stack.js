...
exports.createStack = async (config, validStacks) => {
  ...
  if (config.current.stack.parameters && config.current.stack.parameters.length > 0 && config.current.flags.prerequisite) {
    const requiredStacks = config.current.stack.parameters.filter(p => p.ParameterKey.endsWith('StackName')).map(p => p.ParameterValue);
  ...
  if (config.current.flags.lambda) {
    ...
    const lrs = util.getTemplateLambdaResourcesSummary(config.current.stack.template.body);
    ...
    if (lrs.some(s => s.CodeLocation == 'S3Bucket')) {
      ...
      try {
        ...
        debug('- merging lambda package version parameters');
        // YOU ARE HERE - We need to merge in CalculatedParameters to get the final Parameters
        //              - old: config.initStack(calculatedParameters, calculatedCapabilities);
        process.exit(222); // Force error exit if we hit this point before we implement this merge code
      }
      catch (err) {
        if (err.code && err.code == 'NotFound') {
  ...
  if (createStack) {
    ...
    if (config.current.flags.wait || config.current.flags.monitor) {
      ...
      const describeStackEventsParams = {
        StackName: config.current.stack.name
      };
      const describeStacksParams = {
        StackName: config.current.stack.name
     };
